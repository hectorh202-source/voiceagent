...
const insertStmt = db.prepare(
  `INSERT INTO users (email, password_salt, password_hash) VALUES (@email, @salt, @hash)`,
);
...
export function createUser(email: string, password: string): User {
  ...
  const info = insertStmt.run({ email: normalized, salt, hash }) as { lastInsertRowid: number | bigint };
