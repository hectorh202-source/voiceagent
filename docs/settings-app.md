...
1. **Server-side, in `src/index.ts`** — every other `/app/*` path is served unconditionally (the SPA shell, then client-side `AuthGate` checks `/api/session` after load), which is fine for pages where a brief loading flash isn't a real disclosure. `/app/admin` is the one exception: a `GET /app/admin` handler, registered before the SPA's catch-all, checks `req.session.userId` against `getUserById()` and redirects to `/app` immediately — before any HTML is ever sent — if the session doesn't belong to a platform admin. This means a non-admin's browser never actually resolves that URL at all, the same way `requirePlatformAdmin` already gates `/settings` server-side; there's no window where the address bar shows `/app/admin` with real (even if blank) content behind it.
