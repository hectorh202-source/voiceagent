...
**The same console also exists in-app**, as `client/src/pages/AdminSettingsPage.tsx` at `/app/:businessId/admin` — linked from the sidebar (`AppShell.tsx`) only when `req.currentUser.isPlatformAdmin` is true, so a scoped user never even sees the link. It's a straight JSON port of the same actions (business list/create, user list/create/delete, the same access-assignment checkboxes), backed by a parallel API in `src/api/adminRouter.ts`, gated by `requireApiPlatformAdmin` (`src/api/requireApiPlatformAdmin.ts` — the same check as `requirePlatformAdmin`, just a `403` JSON response instead of a redirect, matching the existing `requireAdminSession`/`requireApiSession` split). The page also checks `isPlatformAdmin` itself before rendering anything (not just relying on the sidebar link being hidden), so navigating there directly by URL as a non-admin shows a plain "you don't have access" message rather than a broken page. The original server-rendered `/settings` console is untouched and still fully functional — not a replacement, a second UI over the same underlying data.
