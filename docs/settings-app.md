...
- **`/app/:businessId/admin`** (nested, reached by picking a business from the switcher while anywhere in the Admin section) — that one business's **Users**: every non-admin user currently assigned to it, each with a **Remove** button that unassigns *only this business* (`DELETE /api/admin/businesses/:businessId/users/:userId` — the account and any other business's access stays untouched, confirmed via a real test: removing a user this way left their row intact with `businessIds: []`, not deleted). Below that, an "Add a user" form creates a **brand-new** account scoped only to this business (`POST /api/admin/businesses/:businessId/users`) — there's no way to grant an *existing* user (one already working at a different business) access to a second business from this UI; that's a deliberate scope cut, not an oversight. Below the Users section, the page embeds `GeneralSettingsPage` directly (the same component previously reached via its own nav link — see below) — this business's ElevenLabs/ServiceTitan credentials and operational settings, now only reachable from here.
  ...
  **A real, confirmed consequence of "Remove" only unassigning, not deleting**: a business-scoped user removed from their only business ends up with `businessIds: []` — invisible on *every* business's own Users card (each one filters to `businessIds.includes(thisBusiness)`) and invisible in Platform Admins too (filters to `isPlatformAdmin`), with no page in the app showing them at all. The account still occupies that email in the `users` table's unique constraint, so re-adding the same email (thinking the account was fully gone) fails with a `409 "That email is already in use."` — confirmed as a real reported bug, not a hypothetical. Fixed by the **All Users** section below, not by changing what "Remove" does here (unassign-only is still the right default action for this button).
...
**Two different actions, easy to conflate**:
- **Unassign from one business** — that business's own Users card → **Remove**. Only deletes the `user_businesses` row; the account (and any other business's access) stays intact. The right everyday action for "this person no longer works with this particular business."
- **Delete the account entirely** — `/app/admin` → **All Users** (business-agnostic; lists every account, admin or not, along with which businesses it belongs to) → **Delete account**. This is the real fix for the scenario above: an account with "No businesses — orphaned" next to it is exactly a user someone unassigned from their last business without realizing that left the account itself behind — delete it from here to free up its email for reuse. (Full admin accounts can also be deleted the same way from the **Platform Admins** section specifically, which predates All Users and still only lists admins — All Users is the one place that covers every account regardless of admin status.) You can't delete your own currently-logged-in account this way (`DELETE /api/admin/users/:id` rejects it) — log in as a different user to remove one.
