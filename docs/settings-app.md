...
**`/app/admin` is a business-agnostic top-level route** (not nested under `/:businessId`, unlike every other in-app page) — reached either via `/settings` redirecting there, or via the sidebar's "Admin Settings" link (`AppShell.tsx`), shown only when `currentUser.isPlatformAdmin` is true, so a scoped user never even sees the link. `AppShell` itself renders with no business loaded on this route: the business switcher is present (so an admin can jump straight into managing a specific business) but the Channels/Settings nav groups are hidden, since there's no `:businessId` in the URL for them to point at. The page also checks `isPlatformAdmin` itself before rendering anything (not just relying on the sidebar link being hidden), redirecting a non-admin straight to `/` (via `<Navigate to="/" replace />`) rather than rendering any admin-labeled content — this matters specifically for a bfcache-restored `/app/admin` document (e.g. right after logging out and back in as a different, non-admin user and hitting the browser's back button): the restored page reloads fresh (see the bfcache section above), refetches the session as the new user, and this guard sends them on to their own view before any business/user data is ever fetched, without so much as flashing a "you don't have access" message. `src/settings/routes.ts`'s `GET /settings` handler is now nothing more than an auth-gated redirect to `/app/admin` — the old server-rendered business/user console (`renderBusinessListPage` and its four POST handlers) was deleted once this became the SPA's job, matching the same "delete fully-superseded code" pattern used earlier when `renderCallListPage`/`businessRoutes.ts` were deleted during the original React dashboard rebuild.
