...
**Follow-up date fallback**: ServiceTitan requires either a `callReasonId` or a `followUpDate` on every lead — confirmed via a real `400`: `"Follow up date or Call Reason ID is required."` We don't have a real scheduled date from the call (`preferredTiming` is freeform text like "afternoons this week," not an actual date), so when no Call Reason ID resolves (see below), the code defaults `followUpDate` to one day out (`Date.now() + 24h`). This is a hardcoded value, not a `/settings` field — deliberately, since it's only satisfying a ServiceTitan API technicality (any value works; a human confirms the real appointment regardless) rather than a business decision that needs regular tuning, and it becomes moot entirely once a Call Reason resolves. If it ever needs to change, it's a one-line edit at the top of the `followUpDate` calculation in [`servicetitan/leads.ts`](../src/servicetitan/leads.ts).

**Call Reason, configurable by name — [`servicetitan/callReasons.ts`](../src/servicetitan/callReasons.ts).** Originally this was just a raw numeric `defaultCallReasonId` someone had to already know and paste in — unlike Business Unit/Job Type/Campaign IDs, ServiceTitan's own dashboard doesn't surface Call Reason IDs anywhere either, so in practice this field mostly just sat blank (falling back to the `followUpDate` technicality above) rather than actually categorizing anything. Fixed the same way Tags already were: resolve by **name** against ServiceTitan's real Call Reasons list, not an opaque ID.

**The real endpoint is not under `/crm/v2/` despite `callReasonId` being a `/crm/v2/.../leads` field** — confirmed against the real OpenAPI spec (`tenant-jbce-v2.json`, module title "Job Booking"):
```
GET /jbce/v2/tenant/{tenantId}/call-reasons
  → paginated { page, pageSize, hasMore, totalCount, data: [{ id, name, isLead, active, createdOn, modifiedOn }] }
```
`findCallReasonIdByName(businessId, name)` mirrors `findTagTypeIdByName()` exactly (case-insensitive exact match, no caching — lead creation is infrequent enough that an extra read call is cheap and means a renamed/newly-added reason in ServiceTitan is picked up immediately). **Confirmed working end-to-end against TitanZ's real account** (2026-08-04): 20 real call reasons returned, ranging from lead-worthy ones (`isLead: true` — "Wanted pricing over the phone," "Will call back," "No Availability") to non-lead ones ("Hang up," "Spam," "Vendor/marketing") — a real, useful distinction a business could use to pick a meaningful default rather than a blind ID. Both the exact-match and not-found (`null`) paths were verified against this real data.
...
**Once set, it wins the displayed "Reason"/"Call Reason" column everywhere** — the Call Detail page's summary "Reason" field and the Calls list table's "CALL REASON" column both show `serviceTitanCallReason ?? callReasonOverride ?? autoCallReason` (see `parseCallRow` in `businessRouter.ts` and `buildCallDetailViewModel` in `dashboard/callDetails.ts`). A deliberate human pick here is treated as more authoritative than either the AI's own classification or the AI-taxonomy manual override, so it takes display priority over both once set. The AI-taxonomy "Call Reason" dropdown itself is untouched — still there, still editable — its value just stops being what's *shown* once a ServiceTitan Call Reason is picked.

**Written back as a Lead/Job note, not the `callReasonId` field itself — a deliberate design forced by a real API limitation — but only when a Lead/Job actually exists.** ServiceTitan's real update schemas were checked directly against the OpenAPI spec: `PATCH /crm/v2/tenant/{tenantId}/leads/{id}` (`Crm.V2.LeadUpdateRequest`) allows only `campaignId, priority, businessUnitId, jobTypeId, summary, leadCustomerName, leadPhone/Email/Street/Unit/City/State/Zip/Country`; `PATCH /jpm/v2/tenant/{tenantId}/jobs/{id}` (`Jpm.V2.UpdateJobRequest`) allows `customerId, locationId, businessUnitId, jobGeneratedLeadSource, jobTypeId, priority, campaignId, summary, shouldUpdateInvoiceItems, customFields, tagTypeIds, externalData, customerPo, soldById, isAutoDispatched, summaryOfWork, noCharge`. Neither includes `callReasonId` — there is no real way to change a Lead or Job's Call Reason after it's created. When a call did produce a real Lead or Job, selecting a value calls `addLeadNote()`/`addJobNote()` in [`servicetitan/leads.ts`](../src/servicetitan/leads.ts)/[`servicetitan/jobs.ts`](../src/servicetitan/jobs.ts) — `POST .../leads/{id}/notes` / `POST .../jobs/{id}/notes` (`Crm.V2.LeadNoteCreateRequest`/`Jpm.V2.JobNoteCreateRequest`, both just `{ text: string, pinToTop?: boolean | null }`, confirmed against the real spec) — posting a plain note like `ServiceTitan Call Reason (set via AI phone agent dashboard): {name}` to that Lead or Job. **When a call never reached `create_lead`/`book_job`, no note is attempted at all** — there's nowhere to post one — and the value just updates the local column directly.

**The API layer** (`businessRouter.ts`): `GET /calls/:conversationId/servicetitan-call-reasons` returns the business's real list (via the new `listCallReasons()` in `callReasons.ts`, which throws on failure unlike the existing name-lookup helper, so the route can surface a clear error instead of silently returning nothing) for the dropdown; `PUT /calls/:conversationId/servicetitan-call-reason` resolves the call's real `leadId`/`jobId` the same way the call list/detail views already do. If either exists, it posts the note and returns `502` on failure without persisting anything; if neither exists, it skips the note entirely and persists directly. Either way, the local value (`elevenlabs_calls.service_titan_call_reason`) is only ever set once whatever write was actually attempted succeeds (or once it's confirmed nothing needed attempting) — so it's never left reflecting an attempted-but-failed push. The dashboard shows a different confirmation line depending on which path ran: "Posted as a note to ServiceTitan." vs. "Saved — no ServiceTitan Lead or Job on this call to attach a note to."

**Live-verified end-to-end against TitanZ's real sandbox account** (2026-08-04), both paths: (1) a call with a real Lead — selected "Wanted pricing over the phone" from the real 20-item list, confirmed the note posted (`addLeadNote()` succeeded), the "Reason" field updated to match, and the "Posted as a note to ServiceTitan" confirmation appeared; (2) a call with no Lead/Job at all — confirmed the dropdown still rendered, selecting "Spam" succeeded with no ServiceTitan API call attempted, the "Reason" field and the Calls list's "CALL REASON" column both updated to "Spam," and the "Saved — no ServiceTitan Lead or Job..." message appeared instead.
...
**Emergency handling, enforced in the backend, not just the system prompt.** An emergency call never trusts the agent's own multi-turn `check_availability` → `book_job` conversation flow to find and book a real slot — that reliability gap is exactly what caused a real past incident (the Emergency Dispatch transfer failure — see [elevenlabs-tools.md](elevenlabs-tools.md)). Instead, `runBookJobFlow()` in `tools/bookJob.ts` itself checks for real near-term capacity server-side:

1. `findEmergencySlot()` calls `checkAvailability()` (same function `check_availability` itself calls) for a fixed, short window — `now` to `now + 24h` (`EMERGENCY_WINDOW_HOURS`), deliberately narrow since "emergency" means "as soon as possible," not capacity.ts's own 14-day `MAX_RANGE_DAYS`. This is also where genuine on-call capacity comes through, if the business has ServiceTitan's own "Include On-Call Technician Shifts in Availability Calculation" Adaptive Capacity setting enabled — confirmed via ServiceTitan's own help docs that this is a real, separate per-Business-Unit setting with no API equivalent (can't be read or set through the API, only ServiceTitan's own admin UI).
2. **A real slot found** → falls through to the exact same booking logic as any other job (`lookupCustomerByPhone`/`createCustomer`/`buildLeadSummary`/`createJob()`), just using the server-found slot instead of an agent-selected one. Logged with a new outcome, `"emergency_job"` — still logged as `toolName: "book_job"` (a real Job really was created), distinct from the "no slot" case for observability.
3. **No real slot found (or the capacity check itself fails)** → falls back to the original safety net exactly as before: `runCreateLeadFlow()` (the same customer-lookup/summary/ServiceTitan-write logic `create_lead` itself uses, shared via `tools/createLead.ts`), logged as `toolName: "create_lead"` — **not** `"book_job"` — so it's found by `findCreateLeadLogByConversationId()` like any other Lead, outcome `"emergency_lead"`.

**Live-verified** (2026-08-07) against TitanZ's real sandbox: with zero configured capacity in that tenant (confirmed via a real `checkAvailability()` call finding no slots even across the full 14-day window — a sandbox/dev-tenant limitation, not a code issue), a real emergency `runBookJobFlow()` call correctly fell through to the Lead safety net and created a real Lead, confirming the refactor didn't change existing behavior when no capacity exists. **Not verified**: the "real slot found → auto-booked as a Job" branch, since this sandbox tenant has no bookable capacity at all to trigger it organically — the logic reuses the exact same `checkAvailability()`/`createJob()` calls already proven elsewhere in this doc, but a real supervised test (a real near-term slot, or a real on-call shift, actually available) is recommended once deployed somewhere with real capacity configured.
