...
**Deferred: pulling equipment age from ServiceTitan for returning customers.** Right now the Age of Equipment line only ever comes from what the agent asks *this* call — there's no equivalent of `getCustomerEmail()` for equipment yet. Unlike contacts/email (every ServiceTitan tenant has those), equipment/install-base tracking varies by tenant setup, so the right API to query hasn't been confirmed against this sandbox yet. When it is, the plan is a `getInstalledEquipmentAge(businessId, customerId, locationId)` in `servicetitan/customers.ts` following the exact same defensive pattern as `getCustomerEmail()` (try/catch, `null` on any failure), feeding into `CustomerLookupResult` the same way `email` does, with the agent's own fresh answer always taking precedence over whatever's on file.
