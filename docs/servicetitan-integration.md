...
**A second, separate "ServiceTitan Call Reason" field on the Call Detail page — not the same thing as the field above.** The `defaultCallReasonName`/`defaultCallReasonId` config above sets one fixed Call Reason automatically applied to *every* lead this business creates. The Call Detail page's own "ServiceTitan Call Reason" dropdown is different: a per-call, staff-picked value from that business's real Call Reasons list — the human-CSR-oriented taxonomy described above — kept deliberately separate from this app's own AI-outcome "Call Reason" dropdown (the `callReason`/`callReasonOverride` fields, populated by ElevenLabs Data Collection) that already exists just above it on the same page. Same English name, two unrelated systems — see the dropdown's placement in `CallDetailPage.tsx` for how they sit side by side without conflating them.

**Written back as a Lead/Job note, not the `callReasonId` field itself — a deliberate design forced by a real API limitation.** ServiceTitan's real update schemas were checked directly against the OpenAPI spec: `PATCH /crm/v2/tenant/{tenantId}/leads/{id}` (`Crm.V2.LeadUpdateRequest`) allows only `campaignId, priority, businessUnitId, jobTypeId, summary, leadCustomerName, leadPhone/Email/Street/Unit/City/State/Zip/Country`; `PATCH /jpm/v2/tenant/{tenantId}/jobs/{id}` (`Jpm.V2.UpdateJobRequest`) allows `customerId, locationId, businessUnitId, jobGeneratedLeadSource, jobTypeId, priority, campaignId, summary, shouldUpdateInvoiceItems, customFields, tagTypeIds, externalData, customerPo, soldById, isAutoDispatched, summaryOfWork, noCharge`. Neither includes `callReasonId` — there is no real way to change a Lead or Job's Call Reason after it's created. Instead, selecting a value calls `addLeadNote()`/`addJobNote()` in [`servicetitan/leads.ts`](../src/servicetitan/leads.ts)/[`servicetitan/jobs.ts`](../src/servicetitan/jobs.ts) — `POST .../leads/{id}/notes` / `POST .../jobs/{id}/notes` (`Crm.V2.LeadNoteCreateRequest`/`Jpm.V2.JobNoteCreateRequest`, both just `{ text: string, pinToTop?: boolean | null }`, confirmed against the real spec) — posting a plain note like `ServiceTitan Call Reason (set via AI phone agent dashboard): {name}` to whichever real Lead or Job this call produced.

**The API layer** (`businessRouter.ts`): `GET /calls/:conversationId/servicetitan-call-reasons` returns the business's real list (via the new `listCallReasons()` in `callReasons.ts`, which throws on failure unlike the existing name-lookup helper, so the route can surface a clear error instead of silently returning nothing) for the dropdown; `PUT /calls/:conversationId/servicetitan-call-reason` resolves the call's real `leadId`/`jobId` the same way the call list/detail views already do, returns `400` if neither exists (nothing to attach a note to), posts the note, and — critically — **only persists the value locally (`elevenlabs_calls.service_titan_call_reason`) once the real ServiceTitan write actually succeeds**, returning `502` otherwise. This makes the stored column an honest signal: if it has a value, ServiceTitan really has that note; it never reflects an attempted-but-failed push. The dashboard UI hides the dropdown entirely (showing "No ServiceTitan Lead or Job for this call") when the call never reached `create_lead`/`book_job`, since there's nothing to attach a note to.

**Live-verified end-to-end against TitanZ's real sandbox account** (2026-08-04): created a real test customer + Lead, called `addLeadNote()` against it directly (succeeded), then exercised the full route through the actual dashboard UI — selected "Wanted pricing over the phone" from the real 20-item list, confirmed the `PUT` request succeeded, the value persisted across a page reload, and the "Posted as a note to ServiceTitan" confirmation text appeared.
