...
A card at the bottom of the call detail page's main column (`CallDetailPage.tsx`) listing every call from the same phone number, newest first, including the call currently being viewed (highlighted with an accent-colored left border). Each row shows the customer name, timestamp, phone, duration, the resolved Bookability status badge, `Emergency`/`Transferred` flag badges when applicable, and a two-line-clamped summary excerpt — clicking any row (other than the current one) navigates to that call's detail page. **Always rendered**, even with zero matches — an empty result shows an explanatory message ("No caller phone number is on file for this call...") rather than the card disappearing; a card that silently vanishes reads as broken, not as "nothing to show."

**Correlation reuses the exact same phone the Calls list already shows — no new column.** `dashboard/callDetails.ts`'s `resolveCallPhone()` parses the same `create_lead`/`book_job` request body the list and this call's own detail view already read `phone` from, computed fresh at request time rather than stored anywhere. `buildCallHistory()` resolves the *current* call's phone this way, then scans every other call in the business (`listCallRecords()`, same function the Calls list itself uses) and keeps the ones that resolve to the same phone.

**Two earlier versions of this got it wrong, in opposite directions worth remembering:**
1. The first version stored a brand-new `caller_phone` column, populated only going forward at webhook-receipt time from an unverified ElevenLabs metadata field, and hid the whole card on zero matches. On real data that field never matched anything, so the card simply never appeared for any call — read as "Call History disappeared," not "nothing to show."
2. A second attempt added fallback sources (a real ElevenLabs dynamic variable, plus the `create_lead`/`book_job` phone) to that same webhook-time column — but a *stored, write-once* column still can't retroactively populate for calls received before the column existed, which is most of a real database on day one. Reusing the phone that's already resolved fresh on every read (this design) sidesteps the whole problem: there's no column to backfill, because nothing is stored — any call, old or new, correlates immediately as long as it (and the candidates being compared against) reached `create_lead`/`book_job`.

A call that never reached a booking tool still has no phone anywhere in this app (same limitation the Calls list and this call's own Phone field already have) — its own Call History card shows the explanatory empty state, and it won't appear in any *other* call's history either.

`buildCallHistory()` in `dashboard/callDetails.ts` does the matching: `listCallRecords()` fetches every call for the business (same function the Calls list uses, capped at 1000 for this lookup), `resolveCallPhone()` is applied to each candidate, and only the ones matching the current call's own resolved phone survive — each surviving row built into a `CallHistoryRow` (reusing the same `create_lead`/`book_job` parsing and `findTransferInfo()` transcript logic `buildCallDetailViewModel` already has), returned as `callHistory` on the `GET /calls/:conversationId` response only — not on the calls list, since a list of every other call there would be redundant with the list itself.

**Performance**: `resolveCallPhone()` calls `findCreateLeadLogByConversationId()`/`findBookJobLogByConversationId()` up to twice per candidate call — so a single call-detail page view can trigger up to ~2000 `call_log` lookups (1000 candidates × 2). Those two functions used to scan every row of `call_log` with an unindexed `request_json LIKE '%...%'` (already flagged as a scaling risk for the flagged-calls-list in [roadmap.md](roadmap.md), which does the same lookup once per row shown there) — multiplying that scan by up to 2000 per page view would have made call-detail pages meaningfully slower as `call_log` grew. Fixed by giving `call_log` its own indexed `conversation_id` column (`src/db/migrateCallLogConversationIdColumn.ts`, backfilled via SQLite's `json_extract` against existing rows' `request_json`, index on `(business_id, conversation_id)`) — both lookup functions now do an exact indexed match instead of a full-table scan, so the cost per lookup no longer grows with total `call_log` size. `logToolCall()` in `db/callLog.ts` populates the column going forward straight from the tool call's own request object (`conversationId` already rides along in every `create_lead`/`book_job` request — see those tools' schemas), no JSON parsing needed for new rows.
