...
`buildCallHistory()` in `dashboard/callDetails.ts` does the matching: `listCallRecords()` fetches every call for the business (same function the Calls list uses), `resolveCallPhone()` is applied to each candidate, and only the ones matching the current call's own resolved phone survive — each surviving row built into a `CallHistoryRow` (reusing the same `create_lead`/`book_job` parsing and `findTransferInfo()` transcript logic `buildCallDetailViewModel` already has), returned as `callHistory` on the `GET /calls/:conversationId` response only — not on the calls list, since a list of every other call there would be redundant with the list itself.
