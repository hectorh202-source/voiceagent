...
### Playback cut off after a few seconds — fixed

The first working recording played only its first ~3 seconds then stopped. Cause: `<audio>` elements stream via HTTP **Range requests** (fetching the file incrementally rather than all at once), and the audio route originally served the whole file with a plain `200` regardless of any `Range` header — browsers that receive a full response when they asked for a partial range can stop playback right where their first requested chunk ended. Fixed in `dashboard/routes.ts`'s `/calls/:conversationId/audio` handler: it now inspects the `Range` header and responds `206 Partial Content` with the exact requested byte range (via `fs.createReadStream(path, { start, end })`) when present, and advertises `Accept-Ranges: bytes` on the full-file response too, so the browser knows partial requests are supported. Verified locally against both a plain request and an explicit `Range: bytes=0-99` request before deploying.
