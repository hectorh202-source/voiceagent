...
- **Call Time is displayed in `America/New_York`, hardcoded** in `dashboard/views.ts`'s `formatCallTime()` — matching the business's Florida location, not derived from any setting. `received_at` is stored as SQLite's `datetime('now')`, which is UTC with no timezone marker; `formatCallTime()` parses it explicitly as UTC before converting for display (`new Date(sqliteDatetime.replace(" ", "T") + "Z")`), then renders via `toLocaleString("en-US", { timeZone: "America/New_York" })` — which conveniently already produces the desired `7/11/2026, 6:24:11 PM` format with no extra formatting options needed. If this business ever operates across multiple timezones, this would need to become a real setting rather than a hardcoded string.
