...
**Correlation requires a new column this app didn't previously capture.** Phone number was only ever available via `create_lead`/`book_job`'s request body — meaning a short call that never reached a tool call (a silent hangup, a wrong number) had no stored phone at all, and so could never be matched to other calls from the same customer. `webhooks/postCall.ts`'s new `extractCallerPhone()` populates a new `caller_phone` column (`src/db/migrateCallerPhoneColumn.ts`) from three sources, tried in order of how confident each one is, so a wrong guess in one doesn't silently break the whole feature:
1. **`conversation_initiation_client_data.dynamic_variables.system__caller_id`** — the same ElevenLabs dynamic variable this app already relies on elsewhere (bound to `lookup_customer`'s `phone` parameter, see [elevenlabs-tools.md](elevenlabs-tools.md)) and confirmed working in production there, so this path has real precedent rather than being a fresh guess.
2. **`metadata.phone_call.external_number`** — ElevenLabs' documented field for a phone-based conversation's caller ID; still unconfirmed against a real payload.
3. **The `create_lead`/`book_job` request body** — the original, pre-existing source. `call_log` is already populated by the time this post-call webhook fires (tool calls happen mid-conversation, this webhook only after the call ends), so this fallback needs no new ElevenLabs configuration and works today for any call that reached a booking tool.

Refreshed on webhook redelivery like `duration_secs`/`call_reason` (not staff-set, so *not* protected from being overwritten the way `is_read`/overrides are).

**A real bug this design fixes**: the first version of this feature had *only* source (2) above, unconfirmed, with the card hidden entirely on zero matches — meaning on real data (where that field almost certainly doesn't match ElevenLabs' actual payload shape) the card simply never appeared for any call, which read as "Call History disappeared" rather than "nothing to show here." Fixed by adding sources (1) and (3) as fallbacks and making the card always render with an explicit empty state. **Still not fully confirmed against a real payload** — place a real test call, inspect the stored `raw_payload_json`, and confirm which of the three sources actually populated `caller_phone` (or adjust `extractCallerPhone()` if none did).
