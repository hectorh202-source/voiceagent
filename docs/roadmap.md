...
- **Fold the remaining global `/settings` pages into the SPA** — login, first-run setup, and migration are still server-rendered HTML (`src/settings/routes.ts`/`views.ts`) and necessarily so (they run before any authenticated SPA session exists). The business-list/platform-user-management console now has an in-app equivalent too (`client/src/pages/AdminSettingsPage.tsx`, `/app/:businessId/admin`, linked from the sidebar for platform admins only — see [settings-app.md](settings-app.md#per-business-access-control--platform-admins-vs-scoped-users)), backed by a parallel JSON API (`src/api/adminRouter.ts`). The original HTML page at `/settings` still exists and still works (same underlying db functions) — not deleted, since it's still what a freshly-logged-in admin lands on by default and costs nothing to keep — but the SPA is now the primary way to manage businesses/users day to day.
