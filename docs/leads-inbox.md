...
**A second, fully independent channel — Microsoft Teams** (`operational.teamsWebhookUrl` + `leadNotifyTeamsEnabled`, same "Notifications" section). One shared webhook URL, separate from email, with its own enabled flag — a business can have email on, Teams on, both, or neither, per notification type (leads vs. calls, see [call-dashboard.md](call-dashboard.md#call-completed-teams-notifications) for the call side). Sent via `sendTeamsMessage()` in `settings/teams.ts` — a plain `POST` of `{ title, text, facts }` JSON to the webhook URL. **Caveat, not fully nailed down**: Microsoft retired the old one-click "Incoming Webhook" connector from the Teams connector gallery; getting a URL today means building a small flow in the Workflows app from the "Post to a channel when a webhook request is received" template, and that flow's own trigger defines whatever JSON schema it expects — there's no one fixed universal schema the way Slack's incoming webhooks have. We send `title`/`text`/`facts` as clearly-named fields a workflow can map, but this hasn't been confirmed against a real Teams channel; the first real setup should double-check the message actually renders as expected and adjust the flow's field mapping if not. Verified independently of email (2026-08-05) via a local mock HTTP receiver standing in for a real Teams webhook URL: confirmed the POST fires with the right shape when the Teams toggle is on and email is off, and doesn't fire at all when the Teams toggle is off (even with email on) — proving the two channels really are independent, not just independently labeled.
