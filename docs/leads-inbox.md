...
`structured_fields` is an optional, encrypted JSON array of `{label, value}` pairs — granular triage details the website chat widget's own assistant records mid-conversation via its `update_state` tool (service type, urgency, preferred timing, etc. — that tool and its logic live entirely in the separate chat-widget repo, not here). `null` for every non-chat source, and for chat leads where the assistant recorded nothing. Sent alongside a chat lead's other fields to the generic webhook below (as a JSON-serializable `structuredFields` array in the request body, validated/bounded by `leadIntakeSchema` — max 40 entries, each field capped at 80/2000 characters so a malformed or oversized payload can't bloat a lead row), stored as a JSON string (`JSON.stringify`'d before `encryptNullable`), and parsed back into an array wherever it's read (`businessRouter.ts`'s `parseStructuredFields` for the API response, `inboundLeads.ts`'s own copy for the notification email below). Rendered on the lead detail page as a "Details captured" list, above the free-text Message section. Not to be confused with ElevenLabs' own "Update state" system tool for the AI *phone* agent (a different product's feature entirely, for a different channel) — the naming is coincidental, not a shared implementation.
...
Every source shares one opt-in email alert, "Email me new leads" (`operational.leadNotifyEnabled`/`leadNotifyEmail`/`leadNotifyCc`, `/app/:businessId/settings/business-info` → "Notifications" section) — website form, website chat, Facebook Ads, Google Ads Lead Form, Google LSA, and the AI phone agent's catch-all, with no per-source distinction. Reachable by any business user, not just platform admins (`GET`/`PUT /settings/notifications` in `businessRouter.ts` carry no `requireApiPlatformAdmin` gate, unlike the rest of General Settings) — deliberately moved out of the admin-only Operational section (2026-08-05) so a business can turn this on/off or change recipients themselves without needing an admin. It's centralized inside `db/inboundLeads.ts`'s `insertInboundLead()` itself, not called separately by each ingestion path — every call to that one function fires it (fire-and-forget, best-effort, never blocking the write or the caller's own response) whenever the lead is genuinely new. Concretely: a plain `INSERT` (no `external_id`, e.g. `website_form`/`website_chat`/`voice_agent`) is always new; an upsert (`external_id` present, e.g. `google_lsa`'s poller) only counts as new the first time that `(business_id, source, external_id)` triple is ever seen — an existence check runs *before* the upsert specifically so a polling source's routine re-fetch of a lead it already recorded never re-sends the alert. `structured_fields` (above) rides along in the email as extra rows, and a `website_chat` lead with `sourceDetail: "booked"` gets a distinct "New appointment booked" heading instead of the generic one, since a real appointment existing is worth saying up front.
...
**A second, fully independent channel — Microsoft Teams** (`operational.teamsWebhookUrl` + `leadNotifyTeamsEnabled`). Split across two places, unlike email: the webhook URL itself is admin-only (`/app/:businessId/admin` → Operational, treated like the other webhook secrets there — masked, "leave blank to keep," never sent back to the client) since it's effectively a bearer credential, while the on/off toggle lives in the business-facing "Notifications" section (`/app/:businessId/settings/business-info`) so any business user can turn posting to Teams on or off without needing (or seeing) the URL. One shared webhook URL, separate from email, with its own enabled flag — a business can have email on, Teams on, both, or neither, per notification type (leads vs. calls, see [call-dashboard.md](call-dashboard.md#call-completed-teams-notifications) for the call side).

**Sent as a real Adaptive Card, not a flat custom JSON body** — `sendTeamsMessage()` in `settings/teams.ts` builds `{ type: "message", attachments: [{ contentType: "application/vnd.microsoft.card.adaptive", content: { $schema: "http://adaptivecards.io/schemas/adaptive-card.json", type: "AdaptiveCard", version: "1.4", body: [...] } }] }` — a title `TextBlock`, a body `TextBlock` (Adaptive Cards support a small Markdown subset here, so the `[label](url)` links both callers already build render as real links with no changes needed on their end), and a `FactSet` for the structured fields. Deliberately built as Adaptive Card rather than the older Office 365 Connector Card/MessageCard format (`@type: "MessageCard"`), since MessageCard is tied to the same connector infrastructure Microsoft is retiring alongside the old one-click Incoming Webhook, while Adaptive Card is what current Workflows templates (paired with a "Post card in a channel" action) are built around.

**Caveat, not fully nailed down**: getting a webhook URL today means building a small flow in the Workflows app from the "Post to a channel when a webhook request is received" template, and that flow's own trigger/action defines exactly how it expects the request body shaped — there's no one fixed universal contract the way Slack's incoming webhooks have, even sending a real Adaptive Card. The schema above matches the public Adaptive Card spec and the envelope both the classic Incoming Webhook and a Workflows "Post card in a channel" action expect, but it hasn't been confirmed rendering correctly in a real Teams channel; the first real setup should double-check the card actually renders as expected.

Verified independently of email (2026-08-05) via a local mock HTTP receiver standing in for a real Teams webhook URL: confirmed the POST fires with the right shape when the Teams toggle is on and email is off, and doesn't fire at all when the Teams toggle is off (even with email on) — proving the two channels really are independent, not just independently labeled. The admin/business split (2026-08-05) was verified too: saving the URL via Admin Settings (`PUT /settings/general`) and reading it back via `getTeamsWebhookUrl()` returned the exact value saved, and the business-facing toggle (`PUT /settings/notifications`) still saves correctly with the URL field gone from that endpoint entirely. The Adaptive Card format itself (2026-08-05) was verified structurally — a real lead insert and a real signed post-call webhook both produced the exact envelope/schema/version/body shape described above, captured via the same mock receiver — though, per the caveat, actual rendering in a live Teams channel is still unconfirmed.

**History**: this used to be two separate, parallel notification paths — website chat had its own older `chatWidget.notifyEnabled`/`notifyEmail`/`notifyCc` setting (on the Chat Widget Settings page) and its own send call, predating the shared design above; every other source had no notification at all. Consolidated into the one setting/path described here once both existed side by side and the duplication became obvious. The old chat-specific settings are retired from the UI, but `isLeadNotifyEnabled()`/`getLeadNotifyEmails()`/`getLeadNotifyCcEmails()` (in `settings/store.ts`) still fall back to reading them when the new `operational.*` keys are unset — a business that had already opted into chat-lead emails keeps getting emailed (now for every lead source, not just chat) rather than that preference silently vanishing. Saving General Settings always writes the new keys going forward.
