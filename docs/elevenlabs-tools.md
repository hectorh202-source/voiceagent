...
**Verify against a real payload before trusting this**, per this project's established discipline: place one real test call after setting this up, then inspect that call's stored `raw_payload_json` (`docker compose exec app node -e "..."`, same one-off-script pattern used elsewhere) to confirm `analysis.data_collection_results.call_reason.value` actually contains what you expect — the exact shape of this field was inferred from ElevenLabs' documented schema, not yet confirmed against a real delivery. If the real payload's structure differs, adjust `extractCallReason()` in `webhooks/postCall.ts` accordingly.
