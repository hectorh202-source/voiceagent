...
3. **Type: String** (not boolean/enum) — this app stores whatever string comes back as-is, no validation against a fixed list.
4. **Prompt/description**: give the LLM the taxonomy of labels you want, e.g. *"Classify this call's outcome as one of: Booked Repair, Unbooked Price Concern, Unbooked Time Concern, Unbooked Reject Agent, Excused Not Qualified Caller, Excused Outside Of Services, Excused No Reason, Follow Up Invoice, Follow Up Cancel, Follow Up Other Update. Pick the single best match."* This list is entirely up to you and isn't hardcoded anywhere in this app — expand/rename freely; whatever string the LLM returns is what shows up in the column verbatim.

**Confirmed against a real payload** (2026-07-15): `analysis.data_collection_results.call_reason.value` arrives exactly as `extractCallReason()` expects — a plain string (e.g. `"slow drip from sink"`) — and the stored `call_reason` column matched it byte-for-byte. Still worth a one-off spot-check (`docker compose exec app node`, piping in a small decrypt-and-print script — same pattern used elsewhere for direct-DB diagnostics) the first time *you* set this field up for a new business, since the label taxonomy/description is entirely up to you and a materially different prompt could in principle produce a different value shape.
