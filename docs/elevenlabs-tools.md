...
2. Description: tell the agent to use the caller's issue to name the closest matching real ServiceTitan job type for this business, e.g. *"The kind of service needed, matching one of this business's real job type names as closely as possible (e.g. 'Plumbing Repair', 'AC Install')."* Exact names matter — the match is case-insensitive but otherwise exact, so a name the agent invents that doesn't match a real job type silently falls back to the single default, no error surfaced. **List this business's real job type names explicitly in the description rather than leaving it to the agent to guess** — confirmed against real TitanZ production logs (2026-08-07) that leaving it to guess reliably produces generic words ("Plumbing", "HVAC") that don't match anything, silently falling back to the same one default job type on every call regardless of the real issue.
...
**Example, TitanZ's real job types (2026-08-07)** — paste the relevant list into the parameter description, e.g.: *"...matching one of this business's real job type names as closely as possible: Drain Cleaning, Plumbing Inspection/Flush, Water Heater, Leak Detection, Repipe, Filtration, Backflow Testing, HVAC No Cool/Heat, HVAC Maintenance, Main Water Service, Drain Clog, HVAC Repair, Misc. Plumbing, HVAC Estimate, HVAC Install, Backflow Install/Repair, Well Service, Commercial HVAC, Commercial Plumbing, Plumbing Install/Repair, Sewer Camera, Drain Repair/Replace, Sewer Relining, Plumbing Estimate."* This list drifts whenever the business adds/renames/removes a ServiceTitan job type — there's no automatic sync into the prompt, so it's worth re-pulling and re-pasting periodically (see the job-types diagnostic in the Debugging section, or just check ServiceTitan's own Job Types settings page).

**Alias fallback, for whatever the prompt still misses.** Even with real names listed, the agent may still land on a generic word sometimes. Business Info's "Service type aliases" field (any business user can edit it, not just admins) lets a business map a phrase directly to one of its real job type names — checked automatically after the exact match fails, before falling back to the single default. For example, mapping `Plumbing` → `Misc. Plumbing` and `HVAC` → `HVAC Repair` catches exactly the generic-word case above without touching the agent's prompt at all.
