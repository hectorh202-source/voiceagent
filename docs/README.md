...
- **[Dynamic memory](dynamic-memory.md)** — opt-in cross-call memory by phone number: a returning caller's agent gets a short summary of their last call. Schema/toggle/write-side hook are built; the live-call delivery webhook is staged behind a manual verification checklist not yet completed.
