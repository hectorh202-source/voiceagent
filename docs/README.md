...
- **[Leads inbox](leads-inbox.md)** — a unified per-business inbox (`/app/:businessId/leads`) aggregating raw inbound leads from website forms/chat (a generic shared-secret webhook), Google Local Services Ads, and Google Ads Lead Form Extensions — with Facebook Lead Ads still deferred — deliberately distinct from a ServiceTitan Lead, dashboard-only, never auto-pushed anywhere.
...
- **[Google Ads Lead Form Extension leads](google-lead-form-leads.md)** — the fourth Leads inbox source, a *different* Google product from LSA with its own real webhook (no OAuth/polling needed), verified against a simulated real payload.
...
- **[Knowledge base](knowledge-base.md)** — one per-business library (typed text, URLs, PDFs/DOCX) shared by **both** the voice agent and the chat widget, edited in one place. This app owns the canonical text and pushes a copy to ElevenLabs; the widget retrieves from a local SQLite FTS5 index. Covers why the direction of truth flipped, the extract-then-review ingestion step, and the two retrieval-quality fixes (porter stemming, term coverage) that testing forced.
